...
- 연쇄율을 통해 결합확률분포를 조건부 확률들의 곱으로 표현할 수 있다. 이것을 Bayes net이라고 하고 유향 비순환 그래프로 기술된다.
- 결합확률분포를 정확히 표현하려면 많은 파라미터가 필요하다. 파라미터 수를 줄이기 위해 근사 모형을 사용한다.
- Bayes net에서 조건부 독립을 가정해 파라미터 수를 줄인다. 예를 들어 naive Bayes는 모든 feature가 독립이라고 가정한다.
- 조건부 확률들을 신경망으로 근사해 파라미터수를 줄일 수도 있다. 예를 들어 FVSBN의 로지스틱 회귀를 1층 신경망으로 바꾼 것이 NADE이다.
